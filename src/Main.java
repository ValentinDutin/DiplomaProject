public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("\nPotential\n");
        MagneticFieldPotential mfp = new MagneticFieldPotential(grid.getG(), N_inside, N_outside, N_surface, grid.getIndexK());
        for(int i = 0; i <= N_surface; i++){
            for(int j = 0; j <= N_inside + N_outside; j++){
                System.out.format("(%f)   ", Math.abs(mfp.getPotential()[i][j]));
            }
            System.out.println();
        }
    }
